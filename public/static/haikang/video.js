...
var Qheight = 0;
var windowtype = 1
function queryheight (height,typenum) {
  Qheight = height;
  windowtype = typenum
}
$(function() {
  ...
  alert('123 :>> ', Qheight);
  // 初始化插件参数及插入插件
  WebVideoCtrl.I_InitPlugin('100%', Qheight, {
...
// 登录szIP,szPort,szUsername,szPassword
function clickLogin(szIPs, szPorts, szUsernames, szPasswords) {
  var szIP = szIPs,
    szPort = szPorts,
    szUsername = szUsernames,
    szPassword = szPasswords;
  ...
  WebVideoCtrl.I_GetDigitalChannelInfo(szIP, {
    ...
    success: function(xmlDoc) {
      ...
      $.each(oChannels, function() {
        ...
        var objTr = $('#digitalchannellist')
          .get(0)
          .insertRow(-1);
        var objTd = objTr.insertCell(0);
        objTd.innerHTML =
          id - iAnalogChannelNum < 10
            ? 'D0' + (id - iAnalogChannelNum)
            : 'D' + (id - iAnalogChannelNum);
...
function clickGetDeviceIP() {
  ...
  szDeviceInfo = WebVideoCtrl.I_GetIPInfoByMode(
    iDeviceMode,
    szAddress,
    iPort,
    szDeviceID
